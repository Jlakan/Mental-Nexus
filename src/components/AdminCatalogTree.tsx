...
export default function AdminCatalogTree() {
  ...
  // Selecciones actuales (CORREGIDO: Usamos CatalogItem para evitar error de tipos)
  const [selProf, setSelProf] = useState<CatalogItem | null>(null);
  const [selCat, setSelCat] = useState<CatalogItem | null>(null);
  const [selSub, setSelSub] = useState<CatalogItem | null>(null);
  ...
  useEffect(() => {
    ...
    setCategories([]); setSubcategories([]); setTasks([]); // Reset hijos
  ...
  useEffect(() => {
    ...
    setSubcategories([]); setTasks([]); // Reset hijos
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{marginBottom:'10px', fontSize:'14px', color:'#555'}}>
        Ruta: 
        <span style={{fontWeight:'bold'}}> {selProf ? selProf.name : "Inicio"} </span>
          ...
          <div style={{marginBottom:'10px'}}>
            {professions.map(p => (
              <div key={p.id} style={{display:'flex', justifyContent:'space-between', marginBottom:'5px'}}>
                <div 
                  onClick={() => { setSelProf(p); setSelCat(null); setSelSub(null); }}
                  style={{
                    background: selProf?.id===p.id?'#2196F3':'white',
                    color: selProf?.id===p.id?'white':'black',
                    padding:'5px', cursor:'pointer', flex:1, borderRadius:'4px', border:'1px solid #eee'
                  }}>
                  {p.name}
                </div>
              ...
              <div style={{marginBottom:'10px'}}>
                {categories.map(c => (
                  <div key={c.id} style={{display:'flex', justifyContent:'space-between', marginBottom:'5px'}}>
                    <div 
                      onClick={() => { setSelCat(c); setSelSub(null); }}
                      style={{
            ...
            <>
              ...
              <div style={{marginBottom:'15px', borderBottom:'1px solid #ddd', paddingBottom:'10px'}}>
                ...
                <div style={{display:'flex', gap:'5px'}}>
                  ...
                  <button onClick={handleAddItem} disabled={loading} style={{background:'#4CAF50', color:'white', border:'none', borderRadius:'4px', cursor:'pointer'}}>
                    + Crear
                  </button>
                </div>
              </div>
              ...
              <div style={{maxHeight:'400px', overflowY:'auto'}}>
                ...
                {tasks.map(t => (
                  <div key={t.id} style={{padding:'8px', borderBottom:'1px solid #eee', background:'white', marginBottom:'5px', fontSize:'13px', borderRadius:'4px', borderLeft: t._collection === 'catalog_routines' ? 
                    '4px solid #9C27B0' : '4px solid #E65100'}}>
                    ...
                    <div style={{display:'flex', justifyContent:'space-between', marginTop:'5px', alignItems:'center'}}>
                      <span style={{
                        ...
                        background: t._collection === 'catalog_routines' ? '#9C27B0' : '#E65100'
                      }}>
                        {t.typeLabel}
                      </span>
                      <button 
                        onClick={() => handleDelete(t.id, t._collection)} 
                        style={{color:'#d32f2f', border:'none', background:'none', cursor:'pointer', fontSize:'14px'}}
                        title="Borrar tarea"
                      >
                        🗑
                      </button>
