...
export function PanelPsicologo({ userData, userUid }: any) {
  ...
  useEffect(() => {
    ...
    return () => { unsubP(); unsubH(); unsubM(); unsubN(); };
  ...
  const guardarNota = async () => {
      ...
      const f = new Date(fechaNota); f.setHours(12,0,0,0);
      await addDoc(collection(db, "users", userUid, "pacientes", pacienteSeleccionado.id, "notas_clinicas"), { contenido: nuevaNota, createdAt: f, autor: userUid });
      setNuevaNota(""); setIndiceNota(0);
  };
  ...
  return (
    <div style={{textAlign: 'left', animation: 'fadeIn 0.3s'}}>
       ...
       {/* MODAL HISTORIAL DE NOTAS (PANTALLA APARTE) */}
       {showHistoryModal && (
       ...
       {activeTab === 'notas' && (
           <div style={{animation:'fadeIn 0.3s'}}>
               <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px'}}>
                   <h3 style={{margin:0, color:'#F8FAFC'}}>BITÁCORA</h3>
                   <div style={{display:'flex', gap:'10px'}}>
                       ...
                       <button onClick={exportarHistorial} style={{background:'none', border:'1px solid #E2E8F0', color:'#E2E8F0', padding:'8px 15px', borderRadius:'8px', cursor:'pointer'}}><IconDownload/></button>
                   </div>
               </div>
